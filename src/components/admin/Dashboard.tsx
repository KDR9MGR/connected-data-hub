...
export function Dashboard() {
  ...
  useEffect(() => {
    (async () => {
      const [blogPublished, blogDrafts, testimonials, portfolio, pricing, treatmentItems, unreadInquiries] = await Promise.all([
      ...
      setCounts({ blogPublished, blogDrafts, testimonials, portfolio, pricing, treatmentItems, unreadInquiries });
    })();
  }, []);
  ...
  const tiles = counts
    ? [
        ...
        { label: "Unread Inquiries", value: counts.unreadInquiries, highlight: counts.unreadInquiries > 0 },
      ]
    : [];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5">
        {tiles.map((t) => (
          <div key={t.label} className={`p-6 rounded-2xl border ${t.highlight ? "border-gold/40 bg-gold/5" : "border-sage/10 bg-stone"}`}>
            <div className={`text-3xl font-serif mb-1 ${t.highlight ? "text-gold" : "text-sage"}`}>{t.value}</div>
