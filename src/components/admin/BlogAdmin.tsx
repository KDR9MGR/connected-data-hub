...
export function BlogAdmin({ userId, isEditor }: { userId: string; isEditor: boolean }) {
  ...
  if (editing)
    return (
      <PostEditor post={editing} userId={userId} onCancel={() => setEditing(null)} onSave={save} />
    );
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3">
        {posts.map((p) => (
          <div
            key={p.id}
            className="bg-stone rounded-2xl p-5 flex flex-wrap items-center justify-between gap-3"
          >
            <div className="min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span
                  className={`text-[9px] uppercase tracking-[0.2em] px-2 py-0.5 rounded-full ${p.status === "published" ? "bg-sage/20 text-sage" : "bg-ink/10 text-ink/60"}`}
                >
                  {p.status}
                </span>
                {p.category && (
...
function PostEditor({
  ...
  return (
    <form
      ...
      className="bg-stone rounded-3xl p-8 space-y-5"
    >
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <F label="Read time">
          <input
            value={p.read_time ?? ""}
            onChange={(e) => up("read_time", e.target.value)}
        ...
        <MediaPicker
          ...
          onChange={(url) => up("cover_image_url", url)}
        />
      </div>
      <F label="Excerpt">
        <textarea
        ...
        <div className="flex gap-2">
          {(["draft", "published"] as const).map((s) => (
            <button
              ...
              type="button"
              onClick={() => up("status", s)}
              className={`px-4 py-2 rounded-full text-[11px] uppercase tracking-[0.18em] ${p.status === s ? "bg-sage text-cream" : "bg-cream border border-sage/20 text-ink/60"}`}
            >
              {s}
            </button>
          ))}
...
const inp =
  "w-full bg-transparent border-b border-sage/20 py-2 text-sm focus:outline-none focus:border-sage";
