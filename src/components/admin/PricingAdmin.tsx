  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-end">
        <button
          onClick={() => setEdit({ features: [], is_published: true, sort_order: 0 })}
      ...
      {edit && (
        <div className="bg-stone rounded-3xl p-8 space-y-5">
          <div className="flex justify-between items-center">
            <h3 className="font-serif text-2xl">{edit.id ? "Edit" : "New"} plan</h3>
            <button
          ...
          <div className="grid md:grid-cols-2 gap-5">
            ...
            <F label="Price label">
              <input
                className={inp}
                value={edit.price_label ?? ""}
                onChange={(e) => setEdit({ ...edit, price_label: e.target.value })}
                placeholder="Rs.2,500 / consult"
              />
            </F>
          </div>
          <F label="Description">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            ...
            <label className="flex items-center gap-2 text-xs text-ink/70 mt-6">
              <input
                type="checkbox"
                checked={edit.is_featured ?? false}
                onChange={(e) => setEdit({ ...edit, is_featured: e.target.checked })}
              />{" "}
              Featured
            </label>
            <label className="flex items-center gap-2 text-xs text-ink/70 mt-6">
          ...
          <div key={p.id} className="bg-stone rounded-2xl p-5 flex flex-wrap justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="text-[10px] uppercase tracking-[0.2em] text-ink/50 mb-1">
                {p.price_label} · {p.is_featured ? "featured · " : ""}
