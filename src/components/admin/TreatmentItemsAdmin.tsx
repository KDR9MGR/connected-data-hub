...
export function TreatmentItemsAdmin({ userId }: { userId: string }) {
  ...
  async function del(id: string) {
    ...
    await supabase.from("treatment_items").delete().eq("id", id); load();
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-end">
        <button onClick={() => setEdit({ pages: [], is_published: true, sort_order: 0 })} className="bg-sage text-cream px-5 py-2.5 rounded-full text-[11px] uppercase tracking-[0.18em] hover:bg-ink">+ Add item</button>
      </div>
      {edit && (
        <div className="bg-stone rounded-3xl p-8 space-y-5">
          <div className="flex justify-between items-center"><h3 className="font-serif text-2xl">{edit.id ? "Edit" : "New"} item</h3>
          ...
          <div className="grid md:grid-cols-2 gap-5">
            ...
            <F label="Category">
              <input className={inp} list="concern-categories" value={edit.category ?? ""} onChange={(e) => setEdit({ ...edit, category: e.target.value })} />
              <datalist id="concern-categories">
                {categories.map((c) => <option key={c} value={c} />)}
              </datalist>
            </F>
          </div>
          <F label="Short description"><textarea rows={2} className={`${inp} resize-none`} value={edit.description ?? ""} onChange={(e) => setEdit({ ...edit, description: e.target.value })} /></F>
          <F label="Full body (optional, for detail views)"><textarea rows={5} className={`${inp} resize-none`} value={edit.body ?? ""} onChange={(e) => setEdit({ ...edit, body: e.target.value })} /></F>
          <MediaPicker userId={userId} label="Media (image, video, or YouTube link)" value={edit.media_url ?? ""} onChange={(url) => setEdit({ ...edit, media_url: url })} />
          <F label="Appears on">
      ...
      <div className="grid gap-3">
          ...
          <div key={t.id} className="bg-stone rounded-2xl p-5 flex flex-wrap justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="text-[10px] uppercase tracking-[0.2em] text-ink/50 mb-1">
                {t.pages.map((p) => PAGE_OPTIONS.find((o) => o.value === p)?.label ?? p).join(", ") || "unassigned"} · {t.is_published ? "published" : "hidden"}
              </div>
        ...
        {items.length === 0 && <div className="text-center text-ink/50 py-16 border border-dashed border-sage/20 rounded-2xl text-sm">No items yet.</div>}
      </div>
    </div>
  );
}
