...
export function SubmissionsAdmin() {
  ...
  async function toggleRead(s: S) {
    await supabase.from("contact_submissions").update({ is_read: !s.is_read }).eq("id", s.id);
    load();
  }
  ...
  return (
    <div className="space-y-3">
      {items.map((s) => (
        <div
          key={s.id}
          className={`rounded-2xl p-5 ${s.is_read ? "bg-stone" : "bg-sage/5 border border-sage/20"}`}
        >
          <div className="flex flex-wrap justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <div className="font-serif">
                {s.name} <span className="text-ink/50 text-sm">· {s.email}</span>
              </div>
            ...
            <div className="flex gap-3 items-start">
              ...
              <button
                ...
                className="text-[11px] uppercase tracking-[0.18em] text-ink/60 border-b border-ink/20 pb-0.5"
              >
          ...
          {openId === s.id && (
            <div className="mt-4 pt-4 border-t border-sage/10 text-sm space-y-2 text-ink/80">
              {s.phone && (
                <div>
                  <span className="text-ink/50 uppercase text-[10px] tracking-[0.2em]">
                    Phone:{" "}
                  </span>
                  {s.phone}
                </div>
