  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-end">
        <button
          onClick={() =>
            setEdit({ media_type: "photo", is_published: true, rating: 5, sort_order: 0 })
          }
      ...
      {edit && (
        <div className="bg-stone rounded-3xl p-8 space-y-5">
          <div className="flex justify-between items-center">
            <h3 className="font-serif text-2xl">{edit.id ? "Edit" : "New"} testimonial</h3>
            <button
          ...
          <div className="grid md:grid-cols-2 gap-5">
            ...
            <F label="Location">
              <input
                className={inp}
                value={edit.author_location ?? ""}
                onChange={(e) => setEdit({ ...edit, author_location: e.target.value })}
              />
            </F>
          </div>
          <F label="Quote">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            ...
            <F label="Media type">
              <select
                className={inp}
                value={edit.media_type ?? "photo"}
                onChange={(e) => setEdit({ ...edit, media_type: e.target.value })}
              >
          ...
          <div key={t.id} className="bg-stone rounded-2xl p-5 flex flex-wrap justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="text-[10px] uppercase tracking-[0.2em] text-ink/50 mb-1">
                {t.media_type} · {t.author_location || "—"} ·{" "}
